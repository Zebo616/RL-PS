...
if __name__ == "__main__":
    ...
    with open("adversaryCoverage.txt") as f:
...
f = open("pokemon-showdown/p2Preferences.txt", "w")
f.write("")
f.close()

# initialize the matchup list
matchups = open("EnemySAR.txt")
...
f = open("pokemon-showdown/p2Preferences.txt", "a")
# For each one of p1's Pokemon, find the best move it can use against each of p2's pokemon
for playerPokemon in p2:
    for enemyPokemon in p1:
        playerChoices = []
        if printTurnOutput:
