...
export class AdversaryReinforcementLearningAI extends BattlePlayer {
		...
		if (request.wait) {
			...
			const choices = request.active.map((active: AnyObject, i: number) => {
				...
				if (switches.length && (!moves.length || this.prng.next() > this.move)) {
					...
					const move = this.chooseMove(request, active, moves, p1p, p2p);
	...
	protected finishMoveChoice(choices) {
		// console.log("made the choice...");
		this.choose(choices.join(`, `));
	}
	...
	protected chooseMove(request: AnyObject, active: AnyObject, moves: {choice: string, move: AnyObject}[], p1p: string, p2p: string): string {
		const fs = require("fs");
		...
		// Get the player and enemy pokemon
		// As a general rule, the reinforcement learning agent will be p1, but regardless
		// we can change this as needed.
		let currentPokemon = p1p
		let enemyPokemon = p2p

		// console.log("player pokemon is: " + p1p);
		// console.log("enemy pokemon is: " + p2p);

		// let enemyPokemon = request.side.pokemon[1].details.split(",")[0];
		...
		// 	}
		// })
		// read in move preferences
		let moveChosen = this.prng.sample(moves).choice;
		let foundEntry = false;
		// console.log("read in: " + data);
		const choices = this.preferences.split(/\r?\n/);
		// console.log("result: " + result);
		choices.forEach(function(choice: string) {
			if(choice.includes(currentPokemon) && choice.includes(enemyPokemon)) {
				let index = choice.charAt(choice.length - 1);
				// console.log("MATCH: " + currentPokemon + " & " + enemyPokemon);
				// console.log("using move " + index);
				// if this index is unavailable, ignore the match and continue with a random move
				if(moves.length >= parseInt(index)) {
					moveChosen = "move " + index;
				}
				foundEntry = true;
			} else {
				// console.log(currentPokemon + " " + enemyPokemon);
				// console.log("'" + choice + "'");
			}
		});
		if(foundEntry == false) {
			// console.log("no matches found for " + currentPokemon + " " + enemyPokemon);
		}
		return moveChosen
	}
