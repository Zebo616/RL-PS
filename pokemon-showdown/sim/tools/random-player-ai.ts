...
export class RandomPlayerAI extends BattlePlayer {
	...
	receiveRequest(request: AnyObject) {
		let currentPokemon = request.side.pokemon[0].details.split(",")[0];
		// console.log("random's active pokemon is '" + currentPokemon + "'");
		const fs = require("fs");
		fs.writeFileSync("randomCurrentPokemon-" + this.turn + ".txt", currentPokemon, function(err){
			if(err){
		...
		if (request.wait) {
			...
			const fs = require("fs");
			fs.writeFileSync("enemyCurrentPokemon.txt", currentPokemon, function(err){
				if(err){
				return console.log("error");
				}
			})
	...
	protected chooseMove(active: AnyObject, moves: {choice: string, move: AnyObject}[]): string {
		...
		// If we can only choose between Mirror Move and Disable, then break :(
		if(moves.length == 2 && 
			((toID(moves[0].choice) == "Mirror Move") || (toID(moves[1].choice) == "Mirror Move")) &&
			((toID(moves[0].choice) == "Disable") || (toID(moves[1].choice) == "Disable"))) {
				return "move 0"
			}
		// console.log("RANDOM'S TURN IS " + this.turn);
		let moveChosen = this.prng.sample(moves).choice;
		while(toID(moveChosen) == "Mirror Move" && toID(moveChosen) == "Disable") {
			// Mirror move is too unique to accurately train with so we'll just ignore it completely
			// console.log("Rerolling mirror move...");
			moveChosen = this.prng.sample(moves).choice;
		}
		// console.log("RANDOM chose: " + toID(moveChosen));
		// console.log("RANDOM is returning: '" + moveChosen + "'")
		return moveChosen
	}
